...
BEGIN {
	...
	freetext_prefix = "<answers>"
	freetext_suffix = "</answers>"
	in_false = 0
}
...
FNR == 1 {
	...
	categories_item = categories_item sprintf(categories_item_template, generate_category(FILENAME))
	question = ""
	explanation = ""
	in_question = 0
	in_answer = 0
...
/^[[:space:]]*[\342\200\230aX]+[[:space:]]+[A-Z][[:space:]]+/ {
	...
	gsub(/^[[:space:]]*[\342\200\230aX]+[[:space:]]*/, "", answers[answer_index])
...
/[[:space:]]+True[[:space:]]*$/ || /^[[:space:]]*True[[:space:]]*$/ {
	...
	gsub(/[[:space:]]+/, " ", answers[answer_index])
	gsub(/^[[:space:]]*[\]\[|_]+[[:space:]]*/, "", answers[answer_index])
	gsub(/^[[:space:]]*\342\200\230a[[:space:]]*/, "", answers[answer_index])
	in_question = 0
	in_answer = 0
	in_explanation = 1
	next
...
function generate_category(filename)
{
...
{
	printf "\t%s", freetext_prefix
	print freetext_suffix
}
...
function printQuestion()
{
	...
	if (questions_seen[question] != 1) {
		...
		if (match(answers[1], /^[[:digit:]]/)) {
			...
			printQuestionPrefix(3, FILENAME)
			printFreeText()
		}
...
function setQuestionPrefix(questionType, filename)
{
	...
	question_prefix = sprintf(question_prefix_template, questionType, generate_category(filename))
}
