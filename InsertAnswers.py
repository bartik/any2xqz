...
def parse_and_add_tag(input_file, output_file):
    ...
    for question in root.getElementsByTagName('question'):
        # Remove the last newline character from the <text> tag value for all <question> tags
        text_tags = question.getElementsByTagName('text')
        if text_tags:
            text_tag = text_tags[0]
            if text_tag.firstChild:
                text_value = text_tag.firstChild.nodeValue.rstrip("\n").strip()
                text_tag.firstChild.nodeValue = text_value
        ...
        # Check if the question type is "1" before modifying or adding the <remark> tag or appending to <text>
